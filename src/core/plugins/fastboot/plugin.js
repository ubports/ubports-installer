   ...
   */
   action__create_logical_partition({ partition, size }) {
    return Promise.resolve().then(() => {
      ...
      this.event.emit(
        "user:write:under",
        "Creating logical partition using fastbootd"
      );
      return this.fastboot.createLogicalPartition(
        partition,
        size
      );
    });
   ...
   */
   action__delete_logical_partition({ partition }) {
    return Promise.resolve().then(() => {
      ...
      this.event.emit(
        "user:write:under",
        "Deleting logical partition using fastbootd"
      );
      return this.fastboot.deleteLogicalPartition(
        partition
      );
    });
  }
   ...
   */
   action__resize_logical_partition({ partition, size }) {
    return Promise.resolve().then(() => {
      ...
      this.event.emit(
        "user:write:under",
        "Resizing logical partition using fastbootd"
      );
      return this.fastboot.resizeLogicalPartition(
        partition,
        size
      );
    });
